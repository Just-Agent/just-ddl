  ...
  const copy = zh
    ? {
      ...
      standards: [
        ...
        '必须公开 `data/items.json`，主分支为 `main`。专题族仓库可以为不同专题提供 `data/{topicId}/items.json`，并在 Hub Topic 里填写 `dataUrl`；指标类专题可额外提供 `metricsUrl`。',
      ...
      hubItems: [
        ...
        '默认读取仓库 `data/items.json`；如果是专题族仓库，请补 `sourceMode: "cluster"`、`clusterId` 和 `dataUrl` 指向该专题自己的公开 JSON；期刊发文量、影响因子等指标快照可补 `metricsUrl`。',
    ...
    : {
      ...
      standards: [
        ...
        'Expose public `data/items.json` on the `main` branch. Topic-family repositories can expose `data/{topicId}/items.json` and set `dataUrl` in the Hub Topic; metric topics can also expose `metricsUrl`.',
      ...
      hubItems: [
        ...
        'The default export is `data/items.json`. For a topic-family repository, add `sourceMode: "cluster"`, `clusterId`, and `dataUrl` pointing to that Topic-specific public JSON; journal metrics and similar snapshots can add `metricsUrl`.',
